...
const UserManagement: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [usersResponse, companiesResponse] = await Promise.all([
        apiService.getUsersByUserRoles(), // Buscar usuários através do endpoint user-roles
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      // Se o role mudou, usar o endpoint user-roles com PUT
      const currentRole = editingUser.company_roles?.[0]?.role || '';
      if (editingUserRole && editingUserRole !== currentRole) {
        const profile = await apiService.getUserProfile();
        const userRoleId = editingUser.company_roles?.[0]?.id;
        
        if (!userRoleId || !profile.company_id) {
          toast({
            title: "Erro",
            description: "Dados necessários para atualização não encontrados",
            variant: "destructive",
          });
          return;
        }

        // Usar o endpoint user-roles/id/ com PUT
        await apiService.updateUserRoleByUserRole(userRoleId, {
          user: editingUser.id,
      ...
      // Se outros dados mudaram, atualizar o usuário também
      const hasOtherChanges = 
        updateForm.first_name !== editingUser.first_name ||
        updateForm.last_name !== editingUser.last_name ||
        updateForm.email !== editingUser.email ||
        updateForm.cpf !== editingUser.cpf ||
        updateForm.is_active !== editingUser.is_active;
  ...
  const handleResetPassword = async (userId: number) => {
    try {
      // Primeiro, vamos buscar o email do usuário
      ...
      // Usar o mesmo endpoint do login (auth_reset) que funciona
  ...
  const handleUpdateUserRole = async (userId: number, newRole: string) => {
    try {
      const user = users.find(u => u.id === userId);
      if (!user || !user.company_roles?.[0]) return;

      const currentRole = user.company_roles[0];
      await apiService.updateUserRole(currentRole.id, {
        ...currentRole,
        role: newRole as 'company_admin' | 'employee' | 'client'
      });
  ...
  const openEditDialog = (user: UserList) => {
    setEditingUser(user);
    setEditingUserRole(user.company_roles?.[0]?.role || '');
    setUpdateForm({
